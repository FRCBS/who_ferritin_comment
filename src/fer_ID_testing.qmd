...
# This script assumes the following file structure when saving results
# project/results/figures/ for figures
# project/results/boot/ for bootstrap objects
# project/results/models/ for model objects
# But you may choose to have a different structure
FIGUREPATH <- "~/ferproj/results/figures/"
BOOTPATH <- "~/ferproj/results/boot/"
DATAPATH <- "~/ferproj/data/" # from where you load your data
MODELPATH <- "~/ferproj/results/models/"
knots <- 3:25 # select range of knot numbers to test
response <- "Hemoglobin" # select response variable
zoom <- T # whether scatter plots will be "zoomed in"
boot_n <- 1000 # adjust depending on desired robustness / runtime, low number recommended for testing
```
...
source("~/ferproj/src/functions.R") # load in helper functions
...
# The script also assumes cairo_pdf as the graphical device when saving pdf figures
# You may not necessarily have it on default
```
...
data <- readRDS(paste0(DATAPATH, "YOUR FILTERED AND CLEANED DATA OBJECT :).rds"))
```

## Test RCS model selection

(If plots are too small due to layout settings, you may change the chunk option `layout-ncol: 4` to 3, 2, or 1. You may also view these plots by accessing your FIGUREPATH, or by right-clicking and selecting _Open image in new tab_.)

...
ggplot(data = apparently_healthy, aes(x = Ferritin, y = Hemoglobin)) +
    ...
    labs(title = "Linear piecewise with 'segmented'",
         subtitle = paste("AIC:", round(AIC(segfit), 2)))
```

## Using `mcp`

```{r mcp_fit, cache = TRUE, warning = FALSE}
# Specify model
...
ggplot(data = donors_oi, aes(x = Ferritin, y = Hb)) +
    ...
    scale_x_continuous(limits = c(0, 100)) +
    labs(title = "Linear piecewise with 'segmented' | Haemoglobin",
         subtitle = paste("AIC:", round(AIC(segfit_fd_haem), 2)))
...
ggplot(data = donors_oi, aes(x = Ferritin, y = TransferrinR)) +
    ...
    scale_x_continuous(limits = c(0, 100)) +
    labs(title = "Linear piecewise with 'segmented' | sTfR",
         subtitle = paste("AIC:", round(AIC(segfit_fd_stfr), 2)))
